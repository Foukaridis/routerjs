...
$(document).ready(function() {
    $.extend($.fn.dataTable.defaults, {
        ...
        "fnFooterCallback": function(nFoot, aData, iStart, iEnd, aiDisplay) {
            var pages = Math.ceil(aData.length / 10);
            var cur_page = Math.ceil(iStart / 10) + 1;
            ...
            $('.dataTables_wrapper').find('.dropdown_pagination').html('');
            $('.dataTables_wrapper').find('.dropdown_pagination').append(dd);

            $('.dataTables_wrapper').find('.dropdown_pagination').find('select').uniform();
            $('.dataTables_wrapper').find('.dropdown_pagination').find('select').change(function() {
                $('#tbl_lists_candidates').dataTable().fnPageChange($(this).find('option:selected').attr('value') - 1, true);
        ...
        "sDom": '<"header"<"actions"><"info"i>>rt<"bottom"<"dropdown_pagination"><"button_pagination"p>>'
    });

    $('table').bind('sort', function(e, v /* this */ , e, n, t) {
        $(this).find('th').find('img').remove();
        var $sort_col = $(this).find('th').eq(v.aaSorting[0][0]);
        $sort_col.append('<img src="/img/' + v.aaSorting[0][1] + '.png" />');
    });
});
