function drawGraph(graph_node)
{
	...
	function buildGraph()
	{
		...
		$.ajax({
			url: '/pp/ajax/reports/advtech_activity_graph_summary.php',		// Make an AJAX request to get graph data
...
function bindImageCommands ()
{
	$('a.image-command').each(function()
	{
		$(this).click(function()
		{
			document.InterFilter.action = $(this).attr('href');
			document.InterFilter.target = '_blank';
			document.InterFilter.cmd.value = 'apply_filter';
			document.InterFilter.submit();
			document.InterFilter.action = '';
			document.InterFilter.target = '';
