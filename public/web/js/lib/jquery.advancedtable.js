(function($){
	$.fn.advancedtable = function(custom) {
		...
		if(settings.csvData != ''){
			...
			var data = $(settings.csvData).text();
			var rows = data.split('\n');
			var rowNumber = rows.length;
			var colNumber = rows[0].split(settings.csvSeperator).length;
			var htmlData = "";
		...
		function setActivePage(number){
			...
			// fire custom event on the object passed with options
			if (typeof settings.customEvent !== false)
				$(settings.customEvent.object).trigger(settings.customEvent.event, table);
